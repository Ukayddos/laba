...
public class ShopController {
    ...
    public Viewable basket() {
        ...
        for (Basket item : basketItems) {
            Fruit fruit = catalogRepository.getFruitsById(item.getId());
            item.setFruits(fruit);
        }
    ...
    public void buy(){
        ...
        for (Basket basket : basketItems) {
            Fruit fruit = catalogRepository.getFruitsById(basket.getId());
            if(fruit.getCount() >= basket.getCount()) {
                fruit.setCount(fruit.getCount() - basket.getCount());
                catalogRepository.updateFruits(fruit.getId(), fruit.getName(), fruit.getPrice(), fruit.getCount());
            }
        }
        clearbasket();
    }
